...
TFile *out = new TFile(fileDir+"occupy100mmEvt.root","RECREATE");
//  TH1I* hEneDeposit = new TH1I("hEneDeposit","",17,-0.5,16.5);
  TH2D* hEneDeposit = new TH2D("hEneDeposit","",25,0,50,25,0,50);
  		...
  		for(int s=0;s<16;s++){
			...
			if(EDeps[s]>0){
				...
				//	cout << EDeps[s] << endl;
					...
					hEneDeposit->Fill(hitTimes[s], EDeps[s]);
				//}
