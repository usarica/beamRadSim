...
void PMTWavelengthSpectrum_nondegenerate(double waveMin, double waveMax,int step){
	...
	if (numPhotonTracks>0){
		for (int k=0; k<numPhotonTracks; k++){
			...
			if (wlsParentID.find(trackID+10000000*i)==wlsParentID.end()){
				...
				//time consuming, but the most rigorous method to recover the original spectrum
				if (wls_conv){
					parentID= myROOTEvent->GetPhotonTracks()->at(k)->GetParentID();
...
//						wlsCount[z]++; }
	...
	cout << "Progress: " << i << " out of " << nentries << endl;
   }
